...
% Copyright (c) 2013, University of Nottingham. All rights reserved.
    ...
    if mri_side<csi_info.size(1) && mri_side<csi_info.size(2)
        mri_data = zeros(round(csi_info.size(1)/mri_info.vox(1)),round(csi_info.size(2)/mri_info.vox(2)),round(mri_info.FOV(2)/mri_info.vox(3)));

        ap_s = round((csi_info.size(1)-mri_side)/mri_info.vox(1)/2+1);
        ap_e = round(ap_s+mri_side/mri_info.vox(1)-1);

        rl_s = round((csi_info.size(2)-mri_side)/mri_info.vox(2)/2+1);
        rl_e = round(rl_s+mri_side/mri_info.vox(2)-1);

        mri_data(ap_s:ap_e, rl_s:rl_e,:)= mri_readIMG(mri_fileName);
        
    elseif mri_side<csi_info.size(1)
        mri_data = zeros(csi_info.size(1),mri_side,mri_info.FOV(2));
    ...
    elseif mri_side<csi_info.size(2)
        mri_data = zeros(mri_side,csi_info.size(2),mri_info.FOV(2));
        ...
        ap_s = round((mri_side-csi_info.size(1))/mri_info.vox(1)/2+1);
        ap_e = round(ap_s+csi_info.size(1)/mri_info.vox(1)-1);

        rl_s = round((mri_side-csi_info.size(2))/mri_info.vox(2)/2+1);
        rl_e = round(rl_s+csi_info.size(2)/mri_info.vox(2)-1);
    ...
    if mri_info.FOV(2) > csi_info.size(3)
        fh_s = round((mri_info.FOV(2)-csi_info.size(3))/mri_info.vox(3)/2+1);
        fh_e = round(fh_s+csi_info.size(3)/mri_info.vox(3)-1);
...
%         end
    ...
    x = round(size(mri_data,1)/2-csi_info.size(1)/mri_info.vox(1)/2+linspace(0, double(csi_info.size(1)/mri_info.vox(1)), csi_info.dim(1)+1));
    y = round(size(mri_data,2)/2-csi_info.size(2)/mri_info.vox(2)/2+linspace(0, double(csi_info.size(2)/mri_info.vox(1)), csi_info.dim(2)+1));
    ...
    edges=round(double([size(mri_data,1) size(mri_data,2)])./[csi_info.dim(1) csi_info.dim(2)]);    
